<?php
/**
 * Edit Cohort Page - Custom implementation
 */
...
$PAGE->set_url('/theme/remui_kids/cohorts/edit_cohort.php', array('id' => $cohortid));
$PAGE->set_context($context);
$PAGE->set_title('Edit Cohort');
$PAGE->set_heading('Edit Cohort');
...
// Check if cohort is managed by a plugin (can't be edited)
if (!empty($cohort->component)) {
    print_error('Cohort is managed by ' . $cohort->component . ' and cannot be edited here');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && confirm_sesskey()) {
    ...
    // Check if idnumber already exists (excluding current cohort)
    if (!empty($idnumber)) {
        $existing = $DB->get_record('cohort', array('idnumber' => $idnumber));
        if ($existing && $existing->id != $cohortid) {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $errors[] = 'Error updating cohort: ' . $e->getMessage();
        }
    }
}
...
// Add custom CSS (same as add_cohort.php)
echo "<style>
...
echo "<h1 class='form-title'>Edit Cohort</h1>";
echo "<p class='form-subtitle'>Update cohort information and settings</p>";
echo "</div>";
...
echo "<input type='text' name='name' class='form-input' placeholder='Enter cohort name' required value='" . htmlspecialchars(isset($_POST['name']) ? $_POST['name'] : $cohort->name) . "'>";
...
echo "<input type='text' name='idnumber' class='form-input' placeholder='Enter unique cohort ID' value='" . htmlspecialchars(isset($_POST['idnumber']) ? $_POST['idnumber'] : $cohort->idnumber) . "'>";
...
echo "<textarea name='description' class='form-textarea' placeholder='Enter cohort description'>" . htmlspecialchars(isset($_POST['description']) ? $_POST['description'] : $cohort->description) . "</textarea>";
echo "<div class='form-help'>Provide additional information about this cohort</div>";
echo "</div>";

// Visible field
$visible_checked = isset($_POST['visible']) ? $_POST['visible'] : $cohort->visible;
echo "<div class='form-group'>";
echo "<div class='form-checkbox-group'>";
echo "<input type='checkbox' name='visible' value='1' class='form-checkbox' id='visible' " . ($visible_checked ? 'checked' : '') . ">";
...
echo "<i class='fa fa-save'></i> Update Cohort";
echo "</button>";
