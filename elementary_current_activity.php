 ...
 * 
 * This file handles the current activity page for Grades 1-3 students
...
require_once(__DIR__ . '/middle_functions.php'); // Required for middle school statistics functions
require_login();
...
$PAGE->set_url('/theme/remui_kids/elementary_current_activity.php');
$PAGE->set_title('My Actions - Elementary Dashboard');
$PAGE->set_heading('My Actions');
...
try {
    ...
    error_log("ELEMENTARY_CURRENT_ACTIVITY: Error getting enrolled courses: " . $e->getMessage());
    $courses = [];
}
...
foreach ($courses as $course) {
    try {
            ...
            switch ($cm->modname) {
                    ...
                    try {
                        ...
                        if ($quiz) {
                            ...
                            try {
                                ...
                                error_log("ELEMENTARY_CURRENT_ACTIVITY: Error getting creator: " . $e->getMessage());
                            }
                        ...
                        error_log("ELEMENTARY_CURRENT_ACTIVITY: Error getting quiz details: " . $e->getMessage());
                    }
                ...
                case 'assign':
                    ...
                    try {
                        ...
                        error_log("ELEMENTARY_CURRENT_ACTIVITY: Error getting assignment details: " . $e->getMessage());
                    }
        ...
        error_log("ELEMENTARY_CURRENT_ACTIVITY: Error processing course {$course->id}: " . $e->getMessage());
        continue;
    }
}
...
foreach ($courses as $course) {
    try {
        ...
        error_log("ELEMENTARY_CURRENT_ACTIVITY: Error checking course completion for course {$course->id}: " . $e->getMessage());
        continue;
    }
}
...
if (debugging('', DEBUG_DEVELOPER)) {
    error_log("ELEMENTARY_CURRENT_ACTIVITY: Found " . count($completed_activities) . " completed activities for user {$USER->id}");
    if (count($completed_activities) > 0) {
        error_log("ELEMENTARY_CURRENT_ACTIVITY: Sample activity - " . json_encode(reset($completed_activities)));
    }
}
...
$templatecontext = [
    'custom_elementary_current_activity' => true,
    ...
    'communityurl' => (new moodle_url('/mod/forum/index.php'))->out(),
    'needhelpurl' => (new moodle_url('/theme/remui_kids/need_help.php'))->out(),
    
    // Config wwwroot for Quick Actions fallback URLs
    'config' => [
        'wwwroot' => $CFG->wwwroot
    ],
...
// Render the template
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('theme_remui_kids/elementary_current_activity_page', $templatecontext);
echo $OUTPUT->footer();
