<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * The main codeeditor configuration form
 *
 * @package    mod_codeeditor
 * @copyright  2025 Your Name
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

require_once($CFG->dirroot.'/course/moodleform_mod.php');

/**
 * Module instance settings form
 *
 * @package    mod_codeeditor
 * @copyright  2025 Your Name
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_codeeditor_mod_form extends moodleform_mod {

    /**
     * Defines forms elements
     */
    public function definition() {
        global $CFG;

        $mform = $this->_form;

        // Adding the "general" fieldset, where all the common settings are showed.
        $mform->addElement('header', 'general', get_string('general', 'form'));

        // Adding the standard "name" field.
        $mform->addElement('text', 'name', get_string('codeeditorname', 'codeeditor'), array('size' => '64'));
        if (!empty($CFG->formatstringstriptags)) {
            $mform->setType('name', PARAM_TEXT);
        } else {
            $mform->setType('name', PARAM_CLEANHTML);
        }
        $mform->addRule('name', null, 'required', null, 'client');
        $mform->addRule('name', get_string('maximumchars', '', 255), 'maxlength', 255, 'client');
        $mform->addHelpButton('name', 'codeeditorname', 'codeeditor');

        // Adding the standard "intro" and "introformat" fields.
        $this->standard_intro_elements();

        // Add optional description field with proper editor options
        $editoroptions = array(
            'maxfiles' => EDITOR_UNLIMITED_FILES,
            'noclean' => true,
            'context' => $this->context,
            'subdirs' => true
        );
        $mform->addElement('editor', 'description', get_string('description'), null, $editoroptions);
        $mform->setType('description', PARAM_RAW);
        $mform->addHelpButton('description', 'description', 'codeeditor');

        // ===============================
        // GRADING SECTION
        // ===============================
        // This function automatically adds the "Grade" header with:
        // - Grade Type (Point/Scale)
        // - Maximum grade
        // - Grading method (Simple/Rubric/Marking guide)
        // - Grade category
        // - Grade to pass
        $this->standard_grading_coursemodule_elements();
        
        // Add advanced grading options (like Assignment module)
        
        // Anonymous submissions (Blind marking)
        $mform->addElement('selectyesno', 'blindmarking', get_string('blindmarking', 'assign'));
        $mform->addHelpButton('blindmarking', 'blindmarking', 'assign');
        $mform->setDefault('blindmarking', 0);
        
        // Hide grader identity from students
        $mform->addElement('selectyesno', 'hidegrader', get_string('hidegrader', 'assign'));
        $mform->addHelpButton('hidegrader', 'hidegrader', 'assign');
        $mform->setDefault('hidegrader', 0);
        
        // Use marking workflow
        $mform->addElement('selectyesno', 'markingworkflow', get_string('markingworkflow', 'assign'));
        $mform->addHelpButton('markingworkflow', 'markingworkflow', 'assign');
        $mform->setDefault('markingworkflow', 0);
        
        // Use marking allocation
        $mform->addElement('selectyesno', 'markingallocation', get_string('markingallocation', 'assign'));
        $mform->addHelpButton('markingallocation', 'markingallocation', 'assign');
        $mform->setDefault('markingallocation', 0);
        $mform->hideIf('markingallocation', 'markingworkflow', 'eq', 0);

        // ===============================
        // AVAILABILITY SECTION  
        // ===============================
        $mform->addElement('header', 'availability', get_string('availability', 'assign'));

        // Allow submissions from date
        $mform->addElement('date_time_selector', 'allowsubmissionsfromdate', get_string('allowsubmissionsfromdate', 'assign'), array('optional' => true));
        $mform->addHelpButton('allowsubmissionsfromdate', 'allowsubmissionsfromdate', 'assign');
        $mform->setDefault('allowsubmissionsfromdate', time());

        // Due date
        $mform->addElement('date_time_selector', 'duedate', get_string('duedate', 'assign'), array('optional' => true));
        $mform->addHelpButton('duedate', 'duedate', 'assign');
        $mform->setDefault('duedate', time() + (7 * 24 * 60 * 60));

        // Cut-off date
        $mform->addElement('date_time_selector', 'cutoffdate', get_string('cutoffdate', 'assign'), array('optional' => true));
        $mform->addHelpButton('cutoffdate', 'cutoffdate', 'assign');

        // ===============================
        // SUBMISSION SETTINGS
        // ===============================
        $mform->addElement('header', 'submissionsettings', get_string('submission', 'assign') . ' ' . get_string('settings'));

        // Require students to click submit button
        $mform->addElement('selectyesno', 'requiresubmit', 'Require students to click submit button');
        $mform->addHelpButton('requiresubmit', 'requiresubmit', 'codeeditor');
        $mform->setDefault('requiresubmit', 1);

        // Add standard elements, common to all modules.
        $this->standard_coursemodule_elements();

        // Add standard buttons, common to all modules.
        $this->add_action_buttons();
    }

    /**
     * Preprocess data before form is displayed
     *
     * @param array $defaultvalues
     * @return void
     */
    public function data_preprocessing(&$defaultvalues) {
        // Prepare description editor field
        if (isset($defaultvalues['description'])) {
            $draftideditor = file_get_submitted_draft_itemid('description');
            $defaultvalues['description'] = array(
                'text' => $defaultvalues['description'],
                'format' => isset($defaultvalues['descriptionformat']) ? $defaultvalues['descriptionformat'] : FORMAT_HTML,
                'itemid' => $draftideditor
            );
        }
    }

    /**
     * Add any custom completion rules to the form.
     *
     * @return array Contains the names of the added form elements
     */
    public function add_completion_rules() {
        return array();
    }

    /**
     * Determines if completion is enabled for this module.
     *
     * @param array $data
     * @return bool
     */
    public function completion_rule_enabled($data) {
        return false;
    }
}

