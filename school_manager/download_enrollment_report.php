<?php
/**
 * Download Course Popularity Report - School Manager
 * Export course popularity data based on student enrollments to CSV or Excel
 */
...
foreach ($courses as $course) {
    ...
    $enrollment_status = $total_enrolled > 0 ? 'Active' : 'No Enrollments';
...
usort($enrollment_report_data, function($a, $b) {
    return $b['total_enrolled'] - $a['total_enrolled'];
});

// Calculate statistics
$total_courses = count($enrollment_report_data);
$total_enrollments = array_sum(array_column($enrollment_report_data, 'total_enrolled'));
$average_enrollment = $total_courses > 0 ? round($total_enrollments / $total_courses, 1) : 0;

// Prepare filename
$clean_company_name = preg_replace('/[^a-zA-Z0-9\s-]/', '', $company_info->name);
$clean_company_name = preg_replace('/\s+/', ' ', $clean_company_name);
$filename = $clean_company_name . ' course popularity report.csv';
...
if ($format === 'csv') {
    ...
    // Add report header and summary
    fputcsv($output, ['Course Popularity Report']);
    ...
    fputcsv($output, ['Average Enrollment per Course', $average_enrollment]);
    fputcsv($output, []);
    ...
    $rank = 1;
    foreach ($enrollment_report_data as $row) {
        fputcsv($output, [
            $rank++,
            $row['course_name'],
            $row['short_name'],
