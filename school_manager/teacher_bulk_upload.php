<?php
/**
 * Teacher Bulk Upload Page - Upload multiple teachers at once using CSV files
 */
...
$PAGE->set_url(new moodle_url('/theme/remui_kids/school_manager/teacher_bulk_upload.php'));
$PAGE->set_title('Teacher Bulk Upload - ' . $company_info->name);
$PAGE->set_heading('Teacher Bulk Upload');

// Get statistics - Use proper role-based teacher identification
$current_month_start = strtotime(date('Y-m-01')); // First day of current month
$current_week_start = strtotime('monday this week'); // Start of current week

// Total teachers in school (using role assignments - same logic as Teacher Management)
...
// Active teachers this week (teachers who logged in this week)
$active_teachers_this_week = $DB->count_records_sql(
    "SELECT COUNT(DISTINCT u.id)
     FROM {user} u
     INNER JOIN {company_users} cu ON u.id = cu.userid AND cu.companyid = ? AND cu.managertype = 0
     INNER JOIN {role_assignments} ra ON u.id = ra.userid
     ...
     WHERE u.deleted = 0 
       AND u.suspended = 0
       AND u.lastaccess >= ?",
    [$company_info->id, $current_week_start]
);

// Teachers added this month (based on user creation time)
$teachers_this_month = $DB->count_records_sql(
    "SELECT COUNT(DISTINCT u.id)
     FROM {user} u
     INNER JOIN {company_users} cu ON u.id = cu.userid AND cu.companyid = ? AND cu.managertype = 0
     INNER JOIN {role_assignments} ra ON u.id = ra.userid
     ...
     WHERE u.deleted = 0
       AND u.timecreated >= ?",
    [$company_info->id, $current_month_start]
);

// Statistics calculated (logging disabled for performance)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv_file'])) {
    ...
    if (empty($upload_token) || !isset($_SESSION['last_upload_token']) || $_SESSION['last_upload_token'] !== $upload_token) {
        ...
        $upload_result = process_teacher_csv_upload($_FILES['csv_file'], $company_info, $upload_type, $match_field);
        ...
        redirect($CFG->wwwroot . '/theme/remui_kids/school_manager/teacher_bulk_upload.php?uploaded=1');
        exit;
    } else {
...
if (isset($_GET['uploaded'])) {
    ...
    if (isset($_SESSION['upload_result'])) {
        ...
        // Recalculate statistics after upload to show updated numbers
        $total_teachers = $DB->count_records_sql(
            "SELECT COUNT(DISTINCT u.id)
             FROM {user} u
             INNER JOIN {company_users} cu ON u.id = cu.userid AND cu.companyid = ? AND cu.managertype = 0
        ...
        // Statistics recalculated (logging disabled for performance)
    }
}

function process_teacher_csv_upload($file, $company_info, $upload_type = 'add_new_only', $match_field = 'username') {
    global $DB, $USER, $CFG;
    ...
    $required_columns = ['username', 'password', 'firstname', 'lastname', 'email'];
    $optional_columns = ['phone', 'gradelevel', 'city', 'country', 'notes'];
    ...
    // Processing bulk upload (logging disabled for performance)
    ...
    while (($data = fgetcsv($handle)) !== false) {
            ...
            switch ($upload_type) {
                ...
                case 'update_only':
                    ...
                    try {
                        ...
                        // Assign editingteacher role if not already assigned
                        if ($editingteacher_role && !user_has_role_assignment($existing_user->id, $editingteacher_role->id, $context->id)) {
                            role_assign($editingteacher_role->id, $existing_user->id, $context->id);
                        }
                        
                        // Update custom fields
                        if (!empty($row_data['gradelevel'])) {
                            ...
                            if ($grade_field) {
                                ...
                                if ($existing_grade) {
                                    $existing_grade->data = trim($row_data['gradelevel']);
                                    ...
                                    $grade_data->data = trim($row_data['gradelevel']);
                        ...
                        $error_count++;
                        $errors[] = "Row " . ($success_count + $error_count + 1) . ": Failed to update user - " . $e->getMessage();
                        continue 2;
                    }
            }
        } else {
        ...
        try {
            ...
            $company_user->managertype = 0;
            $company_user->educator = 1; // Mark as educator (teacher)
            $company_user->suspended = 0;
            
            try {
                $DB->insert_record('company_users', $company_user);
            } catch (Exception $e) {
                throw new Exception('Failed to associate teacher with school: ' . $e->getMessage());
            }
            ...
            if (!empty($row_data['notes'])) {
                ...
                if (!$notes_field) {
                    ...
                    $notes_field->description = 'Additional Notes for Teacher';
...
// Custom CSS for the teacher bulk upload layout
echo "<style>";
echo "
...
echo "<h1 class='page-title'>Teacher Bulk Upload</h1>";
...
echo "</div>";
echo "<div class='stat-number'>{$active_teachers_this_week}</div>";
...
echo "</div>";
echo "<div class='stat-number'>{$teachers_this_month}</div>";
...
echo "<h2 class='upload-title'>Bulk Teacher Upload</h2>";
echo "</div>";
echo "<p class='upload-description'>Upload a CSV file to add multiple teachers at once. All teachers will be automatically assigned the 'editingteacher' role and associated with your school.</p>";
...
if ($upload_result) {
    ...
    if ($upload_result['success']) {
        ...
        if (isset($upload_result['success_count']) && $upload_result['success_count'] > 0) {
            $messages[] = "{$upload_result['success_count']} new teacher(s) created";
        }
        if (isset($upload_result['updated_count']) && $upload_result['updated_count'] > 0) {
            $messages[] = "{$upload_result['updated_count']} teacher(s) updated";
        }
        if (isset($upload_result['skipped_count']) && $upload_result['skipped_count'] > 0) {
            $messages[] = "{$upload_result['skipped_count']} teacher(s) skipped";
        }
...
echo "<ul>";
echo "<li><span class='required'>Required columns:</span> username, password, firstname, lastname, email</li>";
echo "<li><span class='optional'>Optional columns:</span> phone, gradelevel, city, country, notes</li>";
echo "<li>File size: Maximum 5MB</li>";
echo "<li>All teachers will be assigned 'editingteacher' role automatically</li>";
echo "<li>All teachers will be associated with your school automatically</li>";
...
echo "<pre>";
echo "username,password,firstname,lastname,email,phone,gradelevel,city,country,notes\n";
echo "john.doe,password123,John,Doe,john.doe@school.com,+1234567890,Grade 9,New York,USA,Math teacher\n";
echo "jane.smith,password456,Jane,Smith,jane.smith@school.com,,Grade 10,Los Angeles,USA,Science teacher\n";
echo "mike.wilson,password789,Mike,Wilson,mike.wilson@school.com,+1987654321,Grade 11,Chicago,USA,English teacher";
echo "</pre>";
echo "</div>";
