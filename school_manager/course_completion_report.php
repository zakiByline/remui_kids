<?php
/**
 * Course Completion Report - School Manager
 * Shows completion statistics for all courses
 */
...
// Get all courses for this company with completion stats
$courses = $DB->get_records_sql(
    "SELECT c.id, c.fullname, c.shortname, c.startdate, c.enddate, c.visible,
            ...
            (SELECT COUNT(DISTINCT cc.userid)
             ...
             WHERE cc.course = c.id AND cc.timecompleted IS NOT NULL) as total_completed
     FROM {course} c
...
$PAGE->set_url('/theme/remui_kids/school_manager/course_completion_report.php');
$PAGE->set_title('Course Completion Report - ' . $company_info->name);
...
<div class="school-manager-main-content">
    <div class="main-content">
        ...
        <div class="page-header">
            <div>
                <h1 class="page-title">Course Completion Report</h1>
                <p class="page-subtitle">Completion statistics for <?php echo htmlspecialchars($company_info->name); ?></p>
            </div>
        ...
        <div class="report-card">
            <h2 class="report-title">Course Completion Statistics</h2>
            ...
            <!-- Courses Table -->
            <table class="report-table" id="completion_table">
                <thead>
                    <tr>
                        <th>Course Name</th>
                ...
                <tbody>
                    ...
                    if (empty($courses)) {
                        ...
                        foreach ($courses as $course) {
                            ...
                            echo "<tr>";
                            echo "<td><a href='{$CFG->wwwroot}/theme/remui_kids/school_manager/course_completion.php?courseid={$course->id}' class='course-name-link'>" . htmlspecialchars($course->fullname) . "</a></td>";
                            echo "<td><span class='enrollment-count'>" . $total_enrolled . "</span></td>";
                            echo "<td>" . $total_completed . "</td>";
                            echo "<td>";
                            echo "<div class='completion-bar'>";
                            echo "<div class='progress-bar'><div class='progress-fill' style='width: {$completion_rate}%'></div></div>";
                            echo "<span class='completion-percent'>{$completion_rate}%</span>";
                            echo "</div>";
                            echo "</td>";
...
function downloadReport() {
    const table = document.getElementById('completion_table');
    let csv = [];
    ...
    link.setAttribute('download', 'course_completion_report_<?php echo date('Y-m-d'); ?>.csv');
