...
// Get all courses for this company with detailed statistics for tabs
$courses_data = [];
if ($company_info) {
...
if ($company_info) {
    $enrollment_courses_data = $DB->get_records_sql(
        "SELECT c.id, c.fullname, c.shortname, c.startdate, c.visible,
               ...
               COUNT(DISTINCT CASE WHEN cc.timecompleted IS NULL AND ula.timeaccess IS NOT NULL THEN ue.userid END) as in_progress_students,
               COUNT(DISTINCT CASE WHEN cc.timecompleted IS NULL AND ula.timeaccess IS NULL THEN ue.userid END) as not_started_students
        FROM {course} c
        ...
        LEFT JOIN {context} ctx ON ctx.id = ra.contextid AND ctx.contextlevel = 50 AND ctx.instanceid = c.id
        LEFT JOIN {role} r ON r.id = ra.roleid
        WHERE cc_link.companyid = :company_id
        AND c.id != 1
        AND (r.shortname = 'student' OR ue.userid IS NULL)
...
$enrollment_total_courses = count($enrollment_courses_data);
...
foreach ($enrollment_courses_data as $course) {
    if ($course->total_students > 0) {
        ...
        $enrollment_total_enrollments += $course->total_students;
    } else {
...
// Get detailed user login activity for students
$student_login_activity = [];
// Get detailed user login activity for teachers
$teacher_login_activity = [];
...
if ($company_info) {
    ...
    // Get detailed student login activity
    $student_login_activity = $DB->get_records_sql(
        "SELECT u.id, u.firstname, u.lastname, u.email, u.timecreated, u.firstaccess, u.lastaccess,
                r.name as rolename, r.shortname as role,
                (SELECT COUNT(*) FROM {logstore_standard_log} WHERE userid = u.id AND action = 'loggedin') as login_count
         FROM {user} u
         INNER JOIN {company_users} cu ON cu.userid = u.id
         INNER JOIN {role_assignments} ra ON ra.userid = u.id
    ...
    // Get detailed teacher login activity
    $teacher_login_activity = $DB->get_records_sql(
        "SELECT u.id, u.firstname, u.lastname, u.email, u.timecreated, u.firstaccess, u.lastaccess,
                r.name as rolename, r.shortname as role,
                (SELECT COUNT(*) FROM {logstore_standard_log} WHERE userid = u.id AND action = 'loggedin') as login_count
         FROM {user} u
         ...
         AND u.deleted = 0
         GROUP BY u.id, u.firstname, u.lastname, u.email, u.timecreated, u.firstaccess, u.lastaccess, r.name, r.shortname
         ORDER BY u.lastname ASC, u.firstname ASC",
        [$company_info->id]
    );
}

// Get user role distribution for this company
$user_distribution = [];
if ($company_info) {
    // Count students
    ...
    // Count teachers (including editing teachers)
    ...
    $user_distribution = [
        ...
        'total' => $students_count + $teachers_count + $managers_count
    ];
}
...
$student_analytics = [
    ...
    'performance_distribution' => [
        'excellent' => 0,  // 80-100% completion
        'good' => 0,       // 60-79% completion
        'average' => 0,    // 40-59% completion
        'poor' => 0        // 0-39% completion
    ]
];
...
$PAGE->set_url('/theme/remui_kids/school_manager/student_performance_report.php');
$PAGE->set_title('Student Performance Report - ' . $company_info->name);
$PAGE->requires->css('/theme/remui_kids/style/remui_kids.css');
...
$sidebarcontext = [
    ...
    'current_page' => 'student_reports',
    'student_report_active' => true,
    'config' => ['wwwroot' => $CFG->wwwroot]
];
echo $OUTPUT->header();
try { echo $OUTPUT->render_from_template('theme_remui_kids/school_manager_sidebar', $sidebarcontext); } catch (Exception $e) { echo '<!-- Sidebar error: ' . $e->getMessage() . ' -->'; }
?>
<style>
.report-table-container { padding: 20px; background: #ffffff; border-radius: 8px; margin-bottom: 30px; }
.student-perf-subtab { transition: all 0.3s ease; }
.student-perf-subtab:hover { color: #3b82f6 !important; background: #f0f9ff; }
.student-perf-subtab.active { color: #3b82f6 !important; border-bottom-color: #3b82f6 !important; }
.studentperf-subtab-content { display: none; }
.studentperf-subtab-content.active { display: block; }
.no-data-row { text-align: center; padding: 80px 20px; color: #9ca3af; }
</style>
<div class='main-container' style='margin-left: 280px; padding: 30px; background: #f9fafb; min-height: 100vh;'>
    <div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 12px; margin-bottom: 30px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);'>
        <div style='display: flex; justify-content: space-between; align-items: center;'>
            <div>
                <h1 style='color: white; font-size: 2rem; font-weight: 700; margin: 0 0 8px 0;'><i class='fa fa-user-graduate'></i> Student Performance Report</h1>
                <p style='color: rgba(255, 255, 255, 0.9); margin: 0; font-size: 1rem;'><?php echo htmlspecialchars($company_info->name); ?></p>
            </div>
            <div><a href='<?php echo $CFG->wwwroot; ?>/theme/remui_kids/school_manager/course_reports.php' style='background: rgba(255, 255, 255, 0.2); color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600;'><i class='fa fa-arrow-left'></i> Back to Reports</a></div>
        </div>
    </div>
            <div id="tab-studentperformance" class="tab-content">
                <div class="report-table-container">
                    <h3 style="font-size: 1.3rem; font-weight: 600; color: #1f2937; margin-bottom: 10px;">
                        <i class="fa fa-user-graduate" style="color: #3b82f6;"></i> Student Performance Report
                    </h3>
                    <p style="color: #6b7280; margin-bottom: 25px; font-size: 0.9rem;">Comprehensive analysis of individual student performance including grades, course completion rates, and test scores.</p>
                    
                    <!-- Student Performance Sub-Tabs Navigation -->
                    <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 30px; border-bottom: 2px solid #e5e7eb; padding-bottom: 0;">
                        <button class="student-perf-subtab active" onclick="switchStudentPerfSubTab('overview')" data-subtab="overview" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #3b82f6; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid #3b82f6; transition: all 0.3s; position: relative; top: 2px;">
                            <i class="fa fa-file-alt"></i> Student Report
                        </button>
                        <button class="student-perf-subtab" onclick="switchStudentPerfSubTab('overviewsummary')" data-subtab="overviewsummary" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #6b7280; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; position: relative; top: 2px;">
                            <i class="fa fa-th-large"></i> Overview
                        </button>
                        <button class="student-perf-subtab" onclick="switchStudentPerfSubTab('academic')" data-subtab="academic" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #6b7280; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; position: relative; top: 2px;">
                            <i class="fa fa-graduation-cap"></i> Academic
                        </button>
                        <button class="student-perf-subtab" onclick="switchStudentPerfSubTab('inactive')" data-subtab="inactive" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #6b7280; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; position: relative; top: 2px;">
                            <i class="fa fa-exclamation-triangle"></i> Inactive student report
                        </button>
                        <button class="student-perf-subtab" onclick="switchStudentPerfSubTab('attendance')" data-subtab="attendance" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #6b7280; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; position: relative; top: 2px;">
                            <i class="fa fa-calendar-check"></i> Attendance
                        </button>
                        <button class="student-perf-subtab" onclick="switchStudentPerfSubTab('engagement')" data-subtab="engagement" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #6b7280; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; position: relative; top: 2px;">
                            <i class="fa fa-comments"></i> Engagement
                        </button>
                        <button class="student-perf-subtab" onclick="switchStudentPerfSubTab('progress')" data-subtab="progress" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #6b7280; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; position: relative; top: 2px;">
                            <i class="fa fa-chart-line"></i> Progress
                        </button>
                        <button class="student-perf-subtab" onclick="switchStudentPerfSubTab('comparison')" data-subtab="comparison" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #6b7280; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; position: relative; top: 2px;">
                            <i class="fa fa-balance-scale"></i> Comparison
                        </button>
                        <button class="student-perf-subtab" onclick="switchStudentPerfSubTab('alerts')" data-subtab="alerts" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #6b7280; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; position: relative; top: 2px;">
                            <i class="fa fa-exclamation-triangle"></i> Alerts
                        </button>
                        <button class="student-perf-subtab" onclick="switchStudentPerfSubTab('dashboard')" data-subtab="dashboard" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 18px; background: none; border: none; color: #6b7280; font-size: 0.9rem; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; position: relative; top: 2px;">
            ...
            <!-- Tab Content: Assessments & Quiz -->
            <div id="tab-assessmentsquiz" class="tab-content">
                <div class="report-table-container">
                    <h3 style="font-size: 1.3rem; font-weight: 600; color: #1f2937; margin-bottom: 10px;">
                        <i class="fa fa-clipboard-check" style="color: #f59e0b;"></i> Assessments & Quiz Analytics
                    </h3>
                    <p style="color: #6b7280; margin-bottom: 30px; font-size: 0.9rem;">Comprehensive quiz and assessment performance analytics for <?php echo htmlspecialchars($company_info->name); ?>.</p>
                    
                    <?php if ($quiz_data['total_quizzes'] > 0): ?>
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; margin-bottom: 40px;">
                            <div style="background: linear-gradient(135deg, #f59e0b15, #d9790615); padding: 25px; border-radius: 12px; border: 1px solid #f59e0b30; box-shadow: 0 2px 8px rgba(245, 158, 11, 0.1);">
                                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px;">
                                    <div style="width: 45px; height: 45px; background: linear-gradient(135deg, #f59e0b, #d97906); border-radius: 10px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(245, 158, 11, 0.2);">
                                        <i class="fa fa-clipboard-list" style="color: white; font-size: 1.2rem;"></i>
                                    </div>
                                    <div>
                                        <div style="font-size: 0.75rem; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600;">Total Quizzes</div>
                                        <div style="font-size: 2rem; font-weight: 800; color: #f59e0b; line-height: 1;"><?php echo $quiz_data['total_quizzes']; ?></div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style="background: linear-gradient(135deg, #3b82f615, #2563eb15); padding: 25px; border-radius: 12px; border: 1px solid #3b82f630; box-shadow: 0 2px 8px rgba(59, 130, 246, 0.1);">
                                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px;">
                                    <div style="width: 45px; height: 45px; background: linear-gradient(135deg, #3b82f6, #2563eb); border-radius: 10px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(59, 130, 246, 0.2);">
                                        <i class="fa fa-tasks" style="color: white; font-size: 1.2rem;"></i>
                                    </div>
                                    <div>
                                        <div style="font-size: 0.75rem; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600;">Total Attempts</div>
                                        <div style="font-size: 2rem; font-weight: 800; color: #3b82f6; line-height: 1;"><?php echo $quiz_data['total_attempts']; ?></div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style="background: linear-gradient(135deg, #10b98115, #05966915); padding: 25px; border-radius: 12px; border: 1px solid #10b98130; box-shadow: 0 2px 8px rgba(16, 185, 129, 0.1);">
                                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px;">
                                    <div style="width: 45px; height: 45px; background: linear-gradient(135deg, #10b981, #059669); border-radius: 10px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(16, 185, 129, 0.2);">
                                        <i class="fa fa-check-circle" style="color: white; font-size: 1.2rem;"></i>
                                    </div>
                                    <div>
                                        <div style="font-size: 0.75rem; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600;">Completed</div>
                                        <div style="font-size: 2rem; font-weight: 800; color: #10b981; line-height: 1;"><?php echo $quiz_data['completed_attempts']; ?></div>
                                    </div>
                                </div>
                            </div>
                            
                            <div style="background: linear-gradient(135deg, #8b5cf615, #7c3aed15); padding: 25px; border-radius: 12px; border: 1px solid #8b5cf630; box-shadow: 0 2px 8px rgba(139, 92, 246, 0.1);">
                                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px;">
                                    <div style="width: 45px; height: 45px; background: linear-gradient(135deg, #8b5cf6, #7c3aed); border-radius: 10px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(139, 92, 246, 0.2);">
                                        <i class="fa fa-star" style="color: white; font-size: 1.2rem;"></i>
                                    </div>
                                    <div>
                                        <div style="font-size: 0.75rem; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600;">Average Score</div>
                                        <div style="font-size: 2rem; font-weight: 800; color: #8b5cf6; line-height: 1;"><?php echo $quiz_data['average_score']; ?>%</div>
                                    </div>
                                </div>
                            </div>
                        </div>
