<?php
/**
 * Edit Teacher - School Manager
 * Form for editing existing teachers in the school
 */
...
// Get teacher data
$teacher = $DB->get_record('user', ['id' => $teacher_id, 'deleted' => 0]);
if (!$teacher) {
    redirect($CFG->wwwroot . '/theme/remui_kids/teacher_management.php', 'Teacher not found.', null, \core\output\notification::NOTIFY_ERROR);
}
...
// Verify teacher belongs to this company
if ($company_info) {
    ...
    if (!$teacher_in_company) {
        redirect($CFG->wwwroot . '/theme/remui_kids/teacher_management.php', 'Access denied. This teacher does not belong to your school.', null, \core\output\notification::NOTIFY_ERROR);
    }
}
...
if ($grade_field) {
    $grade_data = $DB->get_record('user_info_data', ['userid' => $teacher_id, 'fieldid' => $grade_field->id]);
    if ($grade_data) {
        $grade_level = $grade_data->data;
    }
}
...
$PAGE->set_url(new moodle_url('/theme/remui_kids/school_manager/edit_teacher.php', ['id' => $teacher_id]));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if (!empty($_POST['username']) && $DB->record_exists_sql(
            "SELECT 1 FROM {user} WHERE username = ? AND mnethostid = ? AND deleted = 0 AND id != ?",
            [$_POST['username'], $CFG->mnet_localhost_id, $teacher_id]
        )) {
        ...
        if (!empty($_POST['email']) && $DB->record_exists_sql(
            ...
            [$_POST['email'], $CFG->mnet_localhost_id, $teacher_id]
        )) {
        ...
        if (empty($errors)) {
            try {
                ...
                try {
                    ...
                    error_log("Teacher updated successfully with ID: " . $teacher_id);
                    
                    // Handle profile image deletion if requested
                    if (isset($_POST['delete_profile_image']) && $_POST['delete_profile_image'] == '1') {
                        // Create user context
                        $user_context = context_user::instance($teacher_id);
                        
                        // Prepare file area
                        $fs = get_file_storage();
                        
                        // Delete all profile images from both areas
                        $fs->delete_area_files($user_context->id, 'user', 'icon');
                        $fs->delete_area_files($user_context->id, 'user', 'newicon');
                        
                        // Update user's picture field to 0 (no picture)
                        $user = $DB->get_record('user', ['id' => $teacher_id]);
                        $user->picture = 0;
                        $user->timemodified = time();
                        $DB->update_record('user', $user);
                        
                        error_log("✅ Profile image deleted successfully for user ID: " . $teacher_id);
                    }
                    ...
                    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
                        ...
                        if (in_array($uploaded_file['type'], $allowed_types) && $uploaded_file['size'] <= $max_size) {
                            ...
                            if ($stored_file) {
                                ...
                                error_log("✅ Profile image updated successfully for user ID: " . $teacher_id);
                            } else {
                                error_log("❌ Failed to store profile image file for user ID: " . $teacher_id);
                            }
                        } else {
                    ...
                    error_log("Error updating teacher: " . $user_error->getMessage());
                    throw new Exception('Error updating teacher: ' . $user_error->getMessage());
                }
                ...
                if (!empty($_POST['grade_level'])) {
                    ...
                    $existing_grade_data = $DB->get_record('user_info_data', ['userid' => $teacher_id, 'fieldid' => $grade_field_id]);
                    
                    if ($existing_grade_data) {
                        // Update existing grade level
                        $existing_grade_data->data = trim($_POST['grade_level']);
                        $DB->update_record('user_info_data', $existing_grade_data);
                    } else {
                        // Insert new grade level data
                        $grade_data = new stdClass();
                        $grade_data->userid = $teacher_id;
                        $grade_data->fieldid = $grade_field_id;
                        $grade_data->data = trim($_POST['grade_level']);
                        $grade_data->dataformat = FORMAT_HTML;
                        
                        $DB->insert_record('user_info_data', $grade_data);
                    }
                } else {
                    // Delete grade level if empty
                    if ($grade_field) {
                        $DB->delete_records('user_info_data', ['userid' => $teacher_id, 'fieldid' => $grade_field->id]);
                    }
                }
                ...
                $success_message = 'Teacher "' . $user_data->firstname . ' ' . $user_data->lastname . '" has been successfully updated!';
                redirect(
...
// Custom CSS for the edit teacher layout
echo "<style>";
echo "
...
echo "<h1 class='page-title'>Edit Teacher</h1>";
echo "<p class='page-subtitle'>Update information for " . htmlspecialchars(fullname($teacher)) . "</p>";
echo "</div>";
...
$form_action = new moodle_url('/theme/remui_kids/school_manager/edit_teacher.php', ['id' => $teacher_id]);

echo "<form method='POST' action='" . $form_action->out() . "' enctype='multipart/form-data'>";
echo "<input type='hidden' name='sesskey' value='" . sesskey() . "'>";
echo "<input type='hidden' name='delete_profile_image' id='delete_profile_image' value='0'>";
echo "<div class='form-grid'>";

// Username
echo "<div class='form-group'>";
echo "<label for='username' class='form-label'>Username <span class='required'>*</span></label>";
echo "<input type='text' id='username' name='username' class='form-input' value='" . s($teacher->username) . "' required>";
echo "</div>";
...
echo "<input type='text' id='firstname' name='firstname' class='form-input' value='" . s($teacher->firstname) . "' required>";
echo "</div>";

// Last Name
echo "<div class='form-group'>";
echo "<label for='lastname' class='form-label'>Last Name <span class='required'>*</span></label>";
echo "<input type='text' id='lastname' name='lastname' class='form-input' value='" . s($teacher->lastname) . "' required>";
echo "</div>";

// Email
echo "<div class='form-group'>";
echo "<label for='email' class='form-label'>Email Address <span class='required'>*</span></label>";
echo "<input type='email' id='email' name='email' class='form-input' value='" . s($teacher->email) . "' required>";
echo "</div>";

// Phone
echo "<div class='form-group'>";
echo "<label for='phone' class='form-label'>Phone Number</label>";
echo "<input type='tel' id='phone' name='phone' class='form-input' value='" . s($teacher->phone1) . "'>";
echo "</div>";

// Grade Level
echo "<div class='form-group'>";
echo "<label for='grade_level' class='form-label'>Grade Level</label>";
echo "<select id='grade_level' name='grade_level' class='form-select'>";
echo "<option value=''>Select Grade Level</option>";
for ($i = 1; $i <= 12; $i++) {
    $selected = ($grade_level === "Grade $i") ? ' selected' : '';
...
echo "<input type='text' id='city' name='city' class='form-input' value='" . s($teacher->city) . "'>";
echo "</div>";
...
foreach ($countries as $code => $name) {
    $selected = ($teacher->country === $code) ? ' selected' : '';
...
echo "<textarea id='notes' name='notes' class='form-textarea' placeholder='Any additional information about the teacher...'>" . s($teacher->description) . "</textarea>";
echo "</div>";
...
echo "<div class='image-upload-area" . ($teacher->picture ? " has-image" : "") . "' onclick='document.getElementById(\"profile_image\").click()'>";
...
echo "</div>";
echo "<img id='image_preview' class='image-preview' src='" . ($teacher->picture ? $profile_image_url : '') . "' style='display: " . ($teacher->picture ? "block" : "none") . ";' alt='Profile preview'>";
echo "</div>";
echo "<button type='button' class='delete-image-btn' id='delete_image_btn' onclick='deleteImage()' style='display: " . ($teacher->picture ? "block" : "none") . ";'>";
...
function deleteImage() {
    ...
    const deleteFlag = document.getElementById('delete_profile_image');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fileInput.addEventListener('change', function() {
        if (this.files.length > 0) {
            // If a new file is selected, clear the delete flag
            const deleteFlag = document.getElementById('delete_profile_image');
            if (deleteFlag) {
                deleteFlag.value = '0';
            }
