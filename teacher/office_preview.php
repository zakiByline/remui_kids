...
/**
 * Office Preview Endpoint
 * Extracts embedded thumbnail from Excel files (XLSX, ODS) for preview
 * Excel files are ZIP archives that may contain thumbnails in docProps/thumbnail.*
 */
...
if (!secure_file_token::validate($fileid, $userid, $expires, $token)) {
    error_log("Office Preview: Invalid token for fileid={$fileid}, userid={$userid}");
    // Return transparent PNG instead of 403 to avoid console errors
...
if (!$file || $file->is_directory()) {
    error_log("Office Preview: File not found - fileid={$fileid}");
...
if (!in_array($extension, ['xls', 'xlsx', 'ods', 'csv'])) {
    error_log("Office Preview: Invalid file type - fileid={$fileid}, extension={$extension}");
...
$file_url = $CFG->wwwroot . '/theme/remui_kids/teacher/office_preview.php?fileid=' . $fileid . '&userid=' . $userid . '&expires=' . $expires . '&token=' . $token;
error_log("Office Preview: Attempting to extract thumbnail from fileid={$fileid}, filename={$filename}, url={$file_url}");

// Create temporary file for extraction
$temp_file = $CFG->tempdir . '/office_preview_' . $fileid . '_' . time() . '.' . $extension;
...
try {
    ...
    // XLSX and ODS files are ZIP archives - extract thumbnail
    if ($extension === 'xlsx' || $extension === 'ods') {
        // Check if ZipArchive is available
        if (!class_exists('ZipArchive')) {
            error_log("Office Preview: ZipArchive class not available - fileid={$fileid}, filename={$filename}");
        ...
        if ($result !== TRUE) {
            error_log("Office Preview: Failed to open ZIP archive - fileid={$fileid}, filename={$filename}, error_code={$result}");
            throw new Exception("Failed to open ZIP archive: " . $result);
        }
        
        // Look for embedded thumbnail in common locations
        // Excel embeds thumbnails in docProps/thumbnail.*
        $thumbnail_paths = [
        ...
        error_log("Office Preview: ZIP contents sample (first 20) - fileid={$fileid}, files=" . implode(', ', $sample_files));
        
        if (!empty($docprops_files)) {
            error_log("Office Preview: Found potential thumbnail files - fileid={$fileid}, files=" . implode(', ', $docprops_files));
        }
        ...
        foreach ($thumbnail_paths as $thumb_path) {
            ...
            if ($thumbnail_data !== false && strlen($thumbnail_data) > 0) {
                ...
                if (strpos($thumb_path, '.png') !== false) {
                    ...
                    error_log("Office Preview: WMF thumbnail found but not supported - fileid={$fileid}, filename={$filename}, path={$thumb_path}");
                ...
                if ($image_info !== false && $image_info[0] > 0 && $image_info[1] > 0) {
                    ...
                    error_log("Office Preview: Thumbnail found at {$thumb_path} - fileid={$fileid}, filename={$filename}, size=" . strlen($thumbnail_data) . ", dimensions={$image_info[0]}x{$image_info[1]}");
                    break;
                } else {
                    error_log("Office Preview: Invalid image data at {$thumb_path} - fileid={$fileid}, filename={$filename}");
                    $thumbnail_data = false;
                }
            }
        }
        ...
        if ($thumbnail_found && $thumbnail_data !== false) {
            ...
            $image_info = @getimagesizefromstring($thumbnail_data);
            if ($image_info === false) {
                error_log("Office Preview: Invalid thumbnail image data - fileid={$fileid}, filename={$filename}");
            ...
            error_log("Office Preview: No embedded thumbnail found - fileid={$fileid}, filename={$filename}");
            // Return 1x1 transparent PNG instead of 404 to avoid console errors
            $transparent_png = base64_decode('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==');
            header('Content-Type: image/png');
            header('Content-Length: ' . strlen($transparent_png));
        ...
        // Old XLS format or CSV - doesn't have embedded thumbnails
        error_log("Office Preview: Old XLS/CSV format not supported (no embedded thumbnail) - fileid={$fileid}, filename={$filename}");
        // Return 1x1 transparent PNG instead of 404 to avoid console errors
        $transparent_png = base64_decode('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==');
        header('Content-Type: image/png');
        header('Content-Length: ' . strlen($transparent_png));
    ...
    error_log("Office Preview: Exception - fileid={$fileid}, filename={$filename}, error=" . $e->getMessage());
