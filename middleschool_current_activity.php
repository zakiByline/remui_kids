<?php
/**
 * Middle School Current Activity Page
 * 
 * This file handles the current activity page for Grades 4-7 students
...
$PAGE->set_url('/theme/remui_kids/middleschool_current_activity.php');
$PAGE->set_pagelayout('base');
$PAGE->set_title('My Actions');
$PAGE->add_body_class('custom-dashboard-page has-student-sidebar middleschool-current-activity-page');
...
try {
    ...
    error_log("MIDDLESCHOOL_CURRENT_ACTIVITY: Error getting enrolled courses: " . $e->getMessage());
    $courses = [];
}
...
$upcoming_activities = []; // Replaced course_activities with upcoming_activities
$general_activities = [];
$completed_activities = []; // For General tab - completed activities list
...
foreach ($courses as $course) {
    try {
            ...
            switch ($cm->modname) {
                case 'quiz':
                    ...
                    try {
                        ...
                        if ($quiz) {
                            ...
                            // Try to get from logstore or use course teacher
                            $created_by_name = 'Unknown';
                            try {
                                // Try to get creator from logstore_standard_log
                                if ($DB->get_manager()->table_exists('logstore_standard_log')) {
                                    $creator_log = $DB->get_record_sql(
                                        "SELECT u.id, u.firstname, u.lastname
                                         FROM {logstore_standard_log} l
                                         JOIN {user} u ON u.id = l.userid
                                         WHERE l.contextinstanceid = ? 
                                    ...
                                    if ($creator_log) {
                                        $created_by_name = fullname($creator_log);
                                    }
                                }
                                
                                // If no creator found, try to get from course teachers
                                if ($created_by_name === 'Unknown') {
                                    $teacher = $DB->get_record_sql(
                                        "SELECT u.id, u.firstname, u.lastname
                                         ...
                                         JOIN {context} ctx ON ctx.id = ra.contextid AND ctx.contextlevel = ? AND ctx.instanceid = ?
                                         ...
                                         LIMIT 1",
                                        [CONTEXT_COURSE, $course->id, $course->id]
                                    );
                                    if ($teacher) {
                                        $created_by_name = fullname($teacher);
                                    }
                                }
                            } catch (Exception $e) {
                                error_log("MIDDLESCHOOL_CURRENT_ACTIVITY: Error getting creator: " . $e->getMessage());
                            }
                            ...
                            // Calculate best score and last score
                            if (!empty($attempts)) {
                            ...
                            // Determine status based on time open/close and attempts (override if needed)
                            $now = time();
                            ...
                            // Format open and close times in the format: "5 December 2025, 12:30 AM" (without weekday)
                            $timeopen_formatted = '';
                            $timeclose_formatted = '';
                            ...
                            $activity_data['created_by'] = $created_by_name;
                            ...
                            $activity_data['last_score'] = $last_score;
                            $activity_data['status'] = $status;
                            $activity_data['max_grade'] = $quiz->grade;
                        }
                    } catch (Exception $e) {
                        error_log("MIDDLESCHOOL_CURRENT_ACTIVITY: Error getting quiz details: " . $e->getMessage());
                    }
                ...
                case 'assign':
                    ...
                    try {
                        ...
                        if ($assign) {
                            ...
                            if ($submission) {
                                ...
                                if ($submission->status == 'submitted' || $submission->status == 'graded') {
                                    ...
                                    // Format submission time (without weekday)
                            ...
                            if ($grade && $grade->grade !== null) {
                                ...
                                // Format score - show as percentage if max grade is available
                                if ($assign->grade > 0) {
                            ...
                            // Assignment "open" time - use allowsubmissionsfromdate or timemodified (without weekday)
                            if ($assign->allowsubmissionsfromdate) {
                                $timeopen_formatted = userdate($assign->allowsubmissionsfromdate, '%d %B %Y, %I:%M %p');
                            }
                            
                            // Assignment "close" time - use duedate (without weekday)
                            if ($assign->duedate) {
                                $timeclose_formatted = userdate($assign->duedate, '%d %B %Y, %I:%M %p');
                            }
                            
                            // Check if assignment is closed (only if cutoff date has passed)
                            $is_closed = false;
                            if ($assign->cutoffdate > 0 && $assign->cutoffdate < $now) {
                                // Past cutoff date - assignment is closed for new submissions
                        ...
                        error_log("MIDDLESCHOOL_CURRENT_ACTIVITY: Error getting assignment details: " . $e->getMessage());
                    }
        ...
        error_log("MIDDLESCHOOL_CURRENT_ACTIVITY: Error processing course {$course->id}: " . $e->getMessage());
        continue;
    }
}

// Completed activities are now collected during the main loop above
// Also check for completed courses separately
...
foreach ($courses as $course) {
    try {
        ...
        error_log("MIDDLESCHOOL_CURRENT_ACTIVITY: Error checking course completion for course {$course->id}: " . $e->getMessage());
        continue;
    }
}

// Sort quizzes by time (most recent first)
// Use timeopen if available, otherwise use timeclose, otherwise use 0
if (!empty($quiz_activities)) {
    usort($quiz_activities, function($a, $b) {
        // Get sort time - prefer timeopen, fallback to timeclose, otherwise 0
...
// Debug: Log count of completed activities
if (debugging('', DEBUG_DEVELOPER)) {
    error_log("MIDDLESCHOOL_CURRENT_ACTIVITY: Found " . count($completed_activities) . " completed activities for user {$USER->id}");
    if (count($completed_activities) > 0) {
        error_log("MIDDLESCHOOL_CURRENT_ACTIVITY: Sample activity - " . json_encode(reset($completed_activities)));
    }
}
...
$total_general = count($completed_activities); // Use completed activities count for general
$total_activities = $total_quiz + $total_assignment + $total_upcoming + $total_general;

// Prepare template context for the Current Activity page
$templatecontext = [
    'custom_middleschool_current_activity' => true,
...
if ($dashboardtype === 'highschool') {
    ...
    $templatecontext['currentactivityurl'] = new moodle_url('/theme/remui_kids/middleschool_current_activity.php');
    $templatecontext['myreportsurl'] = new moodle_url('/theme/remui_kids/middleschool_myreports.php');
}

// Render the template using Moodle's standard header/footer system
echo $OUTPUT->header();

// Use high school specific template for high school students, otherwise use middle school template
if ($dashboardtype === 'highschool') {
    echo $OUTPUT->render_from_template('theme_remui_kids/highschool_current_activity_page', $templatecontext);
} else {
    echo $OUTPUT->render_from_template('theme_remui_kids/middleschool_current_activity_page', $templatecontext);
}
